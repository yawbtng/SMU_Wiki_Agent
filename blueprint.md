...
Create a deterministic extraction plan before parsing a PDF. The system should first inspect the document quickly, write a `blueprint.md` / blueprint record that decides which pages or page ranges use the fast text parser and which pages use Docling, then parse by following that blueprint exactly.
