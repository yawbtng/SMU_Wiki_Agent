...
def _build_url_scoring_prompt(site_url: str, discovered_rows: list[dict[str, Any]], batch_note: str = "") -> str:
    ...
    return (
        ...
        "Important freshness rule: sitemap lastmod is not enough. If the URL/title is clearly old "
        f"({target_year - 2} or earlier), cap freshness below 40 and score below 70 unless it is a canonical evergreen page.\n"
        "Demote old dated article/story pages, historical event pages, old term course pages, old monthly crime logs, "
        "and yearly recipient/profile pages even when lastmod is recent.\n"
        "Prefer canonical parent pages over dated child pages, for example scholarship parent pages over 2023 scholars pages.\n"
...
def _build_student_useful_url_prompt(
    ...
    return (
        ...
        f"Skip older-year pages when possible, except canonical evergreen core student pages. Treat URL/title years "
        f"{target_year - 2} or earlier as stale even if sitemap lastmod is recent.\n"
        "Demote dated articles, historical events, old term course pages, monthly crime logs, and yearly recipient/profile pages; "
        "prefer their canonical parent pages.\n"
